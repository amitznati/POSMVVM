...
{
    public class DataListViewModel : BindableViewModelBase
    {
        ...
        public DataListViewModel()
        {
            ...
            _eventAggregator.GetEvent<Events.CancelDialog>().Subscribe(DialogCancelExecute);
            _eventAggregator.GetEvent<Events.SelectedEntityChanged>().Subscribe(EditCanExecuteChanged);
            _eventAggregator.GetEvent<Events.SaveDialogComplete>().Subscribe(DialogSaveExecute);
            _eventAggregator.GetEvent<Events.SaveDenied>().Subscribe(SaveDeniedExecute);

            unit = UnitOfWork.get();
            NewCommand = new DelegateCommand(NewExecute);
            EditCommand = new DelegateCommand(EditExecute, CanEditExecute);
            _globals.ActiveType = TabMap[SelectedTabIndex];
        ...
        private void EditCanExecuteChanged(object obj)
        {
        ...
        private static Dictionary<int, Type> TabMap = new Dictionary<int, Type>()
        {
            {0,typeof(Employee)},
            {1,typeof(Product)},
            {2,typeof(Customer)},
        ...
        public int SelectedTabIndex
        {
            ...
            Globals.get().ActiveType = TabMap[SelectedTabIndex];
